...
<div id='beginEnroll' class='hidden max-w-full bg-white mx-auto h-full'>
    <h1 class='text-center font-semibold text-gray-600 my-2'>Biometric scan</h1>
      ...
      <ul class="list-disc">
        ...
        <li>
              2.) <span class='text-xs font-medium''>Scan <span id='counter' class='font-bold'>4</span> times to get accurate image</span>

        </li>
      </ul>
  <di class='flex flex-col justify-start items-center h-full'>
...
<div id='enrollNow' class='max-w-full bg-white mx-auto h-full'>
  ...
  <di class='flex flex-col justify-center items-center h-full'>
      <div id='loader' class='mb-5 hidden' role="status">
        ...
        <button onclick='beginEnroll()' class='text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800'>Enroll</button>
  </div>
...
function storeSample(sample){
    ...
    const attempt =document.getElementById('counter');
         ...
         setTimeout(() => {
            ...
            if (counter === 0) { // Change 'count' to 'counter'
...
function beginEnroll(){
   ...
   const idb = indexedDB.open('user_forms', 1);
	...
	cursor.onsuccess = () => {
        ...
        readyToSubmit(response.value,getSampleFormat);
...
function readyToSubmit(input,sample){
    ...
    xhttp.onreadystatechange = function(){
        if(this.readyState === 4 && this.status === 200){
            if(this.responseText === "success"){
                ...
                showMsgAlert('Enrollment successfully','success');

                 console.log(successMessage, "successsssss");
              
            }
            else{
                
                showMsgAlert(`${this.responseText}`,'error');

                 console.log(`${failedMessage} ${this.responseText}`);
