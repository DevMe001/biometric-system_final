...
<div id='checkDevice' class='max-w-auto bg-white mx-auto h-auto'>
  ...
  <di class='flex flex-col justify-center items-center h-full'>
    ...
    <button onclick='validateConnection()'
      class='text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800'>Check
      the device</button>
</div>
...
<div id='beginEnroll' class='hidden max-w-full bg-white mx-auto h-full'>
  ...
  <ul class="list-disc">
    ...
    <li>
      2.) <span id='getScan' class='text-xs font-medium'>Scan <span id='counter' class='font-bold'>4</span> times to get
        accurate image</span>

    </li>
  </ul>
  <div class='flex flex-col justify-start items-center h-[80%]'>
    <div id='getFingerImage' class='flex flex-row justify-start items-center h-full'></div>
...
<div id='enrollNow' class='max-w-full bg-white mx-auto h-full'>
  ...
  <di class='flex flex-col justify-center items-center h-full'>
    ...
    <button id='btnEnroll' onclick='beginEnroll()'
      class='text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800'>Enroll</button>
</div>
  ...
  let FingerPrintReader = (function () {
    function FingerPrintReader() {
      ...
      this.sdk.onSamplesAcquired = function (s) {
        ...
        document.getElementById('fingerscanner').classList.add('scan');
       ...
       const res = JSON.parse(s.samples);
					...
					let intermidiate = Fingerprint.b64UrlTo64(getBase64);
        ...
        document.getElementById('fingerscanner').classList.remove('scan');

          
					   const baseString = 'data:image/png;base64,' + ctvx;

					   const imgScanner = `<img src=${baseString} alt='finger scan' width='300' height='300'/>`;

						image.innerHTML += imgScanner;


            var file = dataURLtoFile(baseString, 'fingerprint01.png');
            console.log(file);
